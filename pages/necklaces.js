...
// const Necklaces = ({ Products, addToCart, buyNow }) => {
...
// const Necklaces = ({ Products, addToCart, buyNow, wishlist = {}, addToWishlist, removeFromWishlist }) => {
...
//                       <h3 className="text-rose-400 text-xs tracking-[0.2em] uppercase font-light lg:mb-2">NECKLACES</h3>
...
// dynamic icons to reduce initial bundle
...
// main page
const NecklacesPage = ({ Products = {}, addToCart, buyNow, wishlist = {}, addToWishlist, removeFromWishlist }) => {
  ...
  const handleBuyNow = useCallback((product, e) => {
    e?.stopPropagation?.()
    // if you want to call buyNow: if (typeof buyNow === 'function') buyNow(...)
    if (product && product.slug) {
      router.push(`/product/${product.slug}`)
    } else {
      console.warn('Product slug missing for', product)
    ...
    <div className="min-h-screen relative overflow-hidden" style={{ background: 'radial-gradient(circle, #FFF2Ef,#DBC4BF)' }}>
      ...
      <section className="text-gray-600 body-font relative z-10">
        <div className="container px-5 py-20 mx-auto">
          <div className="text-center mb-16">
            <p className="text-rose-400 text-sm tracking-[0.3em] font-light mb-3 uppercase">Exquisite Collection</p>
            <h1 className="text-5xl md:text-6xl font-serif text-slate-800 mb-4 tracking-tight">Necklaces</h1>
            <div className="flex justify-center items-center gap-3 mt-4">
            ...
            <p className="text-slate-600 mt-6 max-w-2xl mx-auto text-lg">
              Discover our handcrafted silver necklaces, each piece designed to add elegance to your style
            </p>
          </div>
          ...
          <div className="text-center mt-8">
            {keys.length === 0 ? (
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-12 max-w-md mx-auto">
                ...
                <h3 className="text-2xl font-serif text-slate-800 mb-3">No Necklaces Available</h3>
                <p className="text-slate-600">Check back soon for new arrivals!</p>
              </div>
            ) : (
...
export async function getServerSideProps(context) {
  ...
  const items = await Product.find({ category: 'Necklaces' }).lean()
  const Necklaces = {}
  for (let item of items) {
    const title = item.title || item._id.toString()
    if (title in Necklaces) {
      if (item.availableQty > 0) {
        if (item.color && !Necklaces[title].color.includes(item.color)) Necklaces[title].color.push(item.color)
        if (item.size && !Necklaces[title].size.includes(item.size)) Necklaces[title].size.push(item.size)
      }
    } else {
      Necklaces[title] = JSON.parse(JSON.stringify(item))
      Necklaces[title].color = item.availableQty > 0 && item.color ? [item.color] : []
      Necklaces[title].size = item.availableQty > 0 && item.size ? [item.size] : []
    }
  }
