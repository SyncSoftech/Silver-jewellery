...
export default function App({ Component, pageProps }) {
  ...
  useEffect(() => {
    ...
    try {
      const rawRecentlyViewed = localStorage.getItem('recentlyViewed')
      if (rawRecentlyViewed) {
        const parsedRecentlyViewed = JSON.parse(rawRecentlyViewed)
        setRecentlyViewed(parsedRecentlyViewed)
      }
    } catch (error) {
      console.error('Error reading recentlyViewed from localStorage:', error)
  ...
  // helper: persist recently viewed to localStorage
  const saveRecentlyViewed = (recentlyViewedArray) => {
    try {
      const arrayToSave = Array.isArray(recentlyViewedArray) ? recentlyViewedArray : []
      localStorage.setItem('recentlyViewed', JSON.stringify(arrayToSave))
    } catch (e) {
      console.error('Error saving recentlyViewed:', e)
    }
  }

  // add to recently viewed (limit to 10 items, most recent first)
  const addToRecentlyViewed = (itemCode, price = 0, name = '', size = '', variant = '', img = '', slug = '') => {
    setRecentlyViewed(prevViewed => {
      // Remove item if it already exists
      const filtered = prevViewed.filter(item => item.itemCode !== itemCode)
      ...
      const limitedViewed = newViewed.slice(0, 10)
  ...
  // buy now from wishlist (temporary storage)
  const buyNowFromWishlist = (itemCode) => {
    if (itemCode in wishlist) {
      const item = wishlist[itemCode]
      const buyNowItem = {}
      buyNowItem[itemCode] = { 
  ...
  const PageContent = (
    <Component
      ...
      clearWishlist={clearWishlist}
      moveToCart={moveToCart}
      buyNowFromWishlist={buyNowFromWishlist}
