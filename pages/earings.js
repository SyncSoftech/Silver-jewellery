...
const AiOutlineHeart = dynamic(() =>
  import("react-icons/ai").then((m) => m.AiOutlineHeart)
);
const AiFillHeart = dynamic(() =>
  import("react-icons/ai").then((m) => m.AiFillHeart)
);
    ...
    <div className="lg:w-1/5  md:w-1/2 w-1/2 p-1 lg:p-4">
      <div className="group relative bg-white rounded-2xl shadow-sm hover:shadow-xl overflow-hidden transition-all duration-500 flex flex-col transform hover:-translate-y-2 cursor-pointer border border-gray-100">
        <div className="relative w-full aspect-square bg-white overflow-hidden">
          ...
          {/* Image wrapper (Link wraps the image area) */}
          <Link href={`/product/${product.slug}`} className="block absolute inset-0">
            <span className="relative block w-full h-full">
              <Image
                ...
                decoding="async"
                priority={false} // keep lazy for below-the-fold
        ...
        <div className="p-4 flex flex-col flex-grow bg-white relative">
          ...
          <h3 className="text-rose-400 text-xs tracking-[0.2em] uppercase font-light lg:mb-2">EARRINGS</h3>
          ...
          <div className="mt-auto">
            <div className="flex items-center justify-between ">
              <div className="flex items-baseline gap-2">
                ...
                {product.oldPrice && <span className="text-slate-400 line-through text-xs">₹{product.oldPrice}</span>}
              </div>
              {product.oldPrice && (
                <span className="text-rose-500 text-xs font-semibold">
...
// Main page
const EarringsPage = ({ Products = {}, addToCart, buyNow, wishlist = {}, addToWishlist, removeFromWishlist }) => {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            setVisibleCount((prev) => Math.min(prev + BATCH, keys.length));
          }
        });
      },
      { rootMargin: "300px" }
      ...
      <section className="text-gray-600 body-font relative z-10">
        <div className="container px-5 py-20 mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-slate-600 mt-6 max-w-2xl mx-auto text-lg">Discover our handcrafted silver earrings, each piece designed to add elegance to your style</p>
          </div>
          ...
          <div className="text-center mt-8">
            {keys.length === 0 ? (
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-12 max-w-md mx-auto">
                ...
                <h3 className="text-2xl font-serif text-slate-800 mb-3">No Earrings Available</h3>
...
export async function getServerSideProps(context) {
  ...
  // fetch earrings from DB
  const items = await Product.find({ category: "Earrings" }).lean();
  const Earrings = {};
  for (let item of items) {
    const title = item.title || item._id.toString();
    if (title in Earrings) {
      if (item.availableQty > 0) {
        if (item.color && !Earrings[title].color.includes(item.color)) Earrings[title].color.push(item.color);
        if (item.size && !Earrings[title].size.includes(item.size)) Earrings[title].size.push(item.size);
      }
    } else {
      Earrings[title] = JSON.parse(JSON.stringify(item));
      Earrings[title].color = item.availableQty > 0 && item.color ? [item.color] : [];
      Earrings[title].size = item.availableQty > 0 && item.size ? [item.size] : [];
    }
  }
