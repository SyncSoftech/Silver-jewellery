...
//                       <h3 className="text-rose-400 text-xs tracking-[0.2em] uppercase font-light lg:mb-2">RINGS</h3>
...
// main page
const RingsPage = ({ Products = {}, addToCart, buyNow, wishlist = {}, addToWishlist, removeFromWishlist }) => {
    ...
    <div className="min-h-screen pt-20  lg:pt-0 relative overflow-hidden" style={{ background: 'radial-gradient(circle, #FFF2Ef,#DBC4BF)' }}>
      ...
      <section className="text-gray-600 body-font relative z-10">
        <div className="container px-5 py-20 mx-auto">
          <div className="text-center mb-16">
            <p className="text-rose-400 text-sm tracking-[0.3em] font-light mb-3 uppercase">Exquisite Collection</p>
            <h1 className="text-5xl md:text-6xl font-serif text-slate-800 mb-4 tracking-tight">Rings</h1>
            <div className="flex justify-center items-center gap-3 mt-4">
            ...
            <p className="text-slate-600 mt-6 max-w-2xl mx-auto text-lg">Discover our handcrafted silver rings, each piece designed to add elegance to your style</p>
          </div>
          ...
          <div className="text-center mt-8">
            {keys.length === 0 ? (
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-12 max-w-md mx-auto">
                ...
                <h3 className="text-2xl font-serif text-slate-800 mb-3">No Rings Available</h3>
                <p className="text-slate-600">Check back soon for new arrivals!</p>
              </div>
            ) : (
...
export async function getServerSideProps(context) {
  ...
  const items = await Product.find({ category: 'Rings' }).lean()
  const Rings = {}
  for (let item of items) {
    const title = item.title || item._id.toString()
    if (title in Rings) {
      if (item.availableQty > 0) {
        if (item.color && !Rings[title].color.includes(item.color)) Rings[title].color.push(item.color)
        if (item.size && !Rings[title].size.includes(item.size)) Rings[title].size.push(item.size)
      }
    } else {
      Rings[title] = JSON.parse(JSON.stringify(item))
      Rings[title].color = item.availableQty > 0 && item.color ? [item.color] : []
      Rings[title].size = item.availableQty > 0 && item.size ? [item.size] : []
    }
  }
