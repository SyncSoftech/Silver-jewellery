...
export default function OrderSuccess() {
  ...
  useEffect(() => {
    ...
    const fetchOrderDetails = async () => {
      try {
        ...
        // fallback to lastOrder in localStorage
        if (lastOrder) {
          // normalize fallback shape to match UI usage and include finalAmount/discount
          const normalized = {
            _id: lastOrder._id || lastOrder.orderId,
            amount: lastOrder.amount || lastOrder.totalAmount || lastOrder.total || 0,
            discountAmount: lastOrder.discountAmount ?? lastOrder.discount ?? lastOrder.discountApplied ?? 0,
            finalAmount: lastOrder.finalAmount ?? ( (lastOrder.amount || lastOrder.total || 0) - (lastOrder.discountAmount ?? lastOrder.discount ?? 0) ),
            status: lastOrder.status || 'Processing',
            createdAt: lastOrder.createdAt || new Date().toISOString(),
            paymentMethod: lastOrder.paymentMethod || 'Online Payment (Razorpay)',
            appliedCoupon: lastOrder.appliedCoupon || (lastOrder.couponCode ? { code: lastOrder.couponCode, discountApplied: lastOrder.discountAmount ?? 0 } : null)
          };
          setOrder(normalized);
          setLoading(false);
          return;
  ...
  const finalVal = typeof order?.finalAmount !== 'undefined'
    ? order.finalAmount
          ...
          <div className="border-t border-gray-200 px-4 py-5 sm:p-0">
            <dl className="sm:divide-y sm:divide-gray-200">
              ...
              <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                ...
                <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">₹{fmt(subtotalVal)}</dd>
              </div>

              {discountVal > 0 && (
                <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                  <dt className="text-sm font-medium text-gray-500">Discount</dt>
                  <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">- ₹{fmt(discountVal)} {order?.appliedCoupon?.code && <span className="text-sm text-green-600 ml-2">({order.appliedCoupon.code})</span>}</dd>
                </div>
              )}

              <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                <dt className="text-sm font-medium text-gray-500">Total amount</dt>
                <dd className="mt-1 text-sm font-medium text-gray-900 sm:mt-0 sm:col-span-2">₹{fmt(finalVal)}</dd>
              </div>
              ...
              <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                ...
                <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                    ...
                    (order?.status || '').toLowerCase().includes('complete') || (order?.status || '').toLowerCase().includes('paid') ? 'bg-green-100 text-green-800' :
                    'bg-gray-100 text-gray-800'
                  }`}>{order?.status || 'Processing'}</span>
                </dd>
              </div>
