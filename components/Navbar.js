...
//   return (
//     <div className='flex flex-col md:flex-row md:justify-start justify-center items-center py-2 shadow-md sticky top-0 bg-white '>
//       <div className="logo md:mx-5 cursor-pointer mr-auto">
//         <Link href={'/'}><Image width={200} height={40} src="/LOGO.jpg"  /></Link>
//       </div>
//       <div className="nav ">
//         <ul className='flex items-center space-x-6 font-semibold md:text-xl'>
//           <Link href={'/bracelets'}><span className='hover:text-blue-600'><li>Bracelets</li></span></Link>
//           <Link href={'/earings'}><span className='hover:text-blue-600'><li>Earings</li></span></Link>
//           <Link href={'/necklaces'}><span className='hover:text-blue-600'><li>Necklaces</li></span></Link>
//           <Link href={'/rings'}><span className='hover:text-blue-600'><li>Rings</li></span></Link>
//         </ul>
//       </div>
//       <div className='cart absolute right-0 mx-5 top-4 cursor-pointer flex'>
//         <span onMouseOver={()=>{setDropdown(true)}} onMouseLeave={()=>{setDropdown(false)}}>
//       {dropdown &&<div onMouseOver={()=>{setDropdown(true)}} onMouseLeave={()=>{setDropdown(false)}} className='absolute right-8 bg-white shadow-2xl  top-7 rounded-md px-5 py-4 w-32'>
//           <ul>
//             <Link href={'/myaccount'}><li className='py-1 hover:text-blue-700  text-sm font-bold'>My Account</li></Link>
//             <Link href={'/orders'}><li className='py-1 hover:text-blue-700  text-sm font-bold'>Orders</li></Link>
//             <span ><li onClick={Logout} className='py-1 hover:text-blue-700  text-sm font-bold'>LogOut</li></span>
//           </ul>
//         </div>}
//         {user.value && <MdAccountCircle  className='text-2xl font-bold md:text-3xl mx-2' />}</span>

//         {!user.value && <Link href={'/login'}><div>
//           <button className='bg-blue-600 px-2 py-1 rounded-md text-sm text-white mx-2'>Login</button>
//         </div></Link>}

//         <CiShoppingCart onClick={toggleCart} className='text-2xl  font-bold md:text-3xl' />
//       </div>
...
//                 <div className='w-1/3 font-semibold flex items-center justify-center text-lg'><AiOutlineMinusCircle onClick={() => { removeFromCart(k, 1, cart[k].price, cart[k].name, cart[k].size, cart[k].variant) }} className='cursor-pointer' /><span className='mx-2 text-sm'>{cart[k].qty}</span><AiOutlinePlusCircle onClick={() => { addToCart(k, 1, cart[k].price, cart[k].name, cart[k].size, cart[k].variant) }} className='cursor-pointer' /></div>
//               </div>
//             </li>
//           })}
...
//         <div className='flex'>
//           <Link href={'/checkout'}><button className="flex  mr-2 text-white bg-indigo-500 border-0 py-2 px-2 focus:outline-none hover:bg-indigo-600 rounded text-sm"><BsBagCheckFill className="m-1" />Checkout</button></Link>
//           <button onClick={clearCart} className="flex  mr-1 text-white bg-indigo-500 border-0 py-2 px-2 focus:outline-none hover:bg-indigo-600 rounded text-sm"><BsBagCheckFill className="m-1" />Clear Cart</button>
//         </div>
//       </div>
